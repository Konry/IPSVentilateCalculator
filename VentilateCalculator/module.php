...
class VentilateCalculator extends IPSModule
{
    ...
    private function CalculateVentilationScaleCo2() : float {
        $co2Mode = GetValue($this->ReadPropertyBoolean('CO2Mode'));
        if($co2Mode == 0){
            return -1;
        }
        $co2YellowRange = GetValue($this->ReadPropertyFloat('CO2YellowLevel'));
        $co2RedRange = GetValue($this->ReadPropertyFloat('CO2RedLevel'));
    ...
    private function CalculateVentilationScaleHumidity() : float {
        $humidityMode = GetValue($this->ReadPropertyBoolean('RelativeHumidityModeActivated'));
        if($humidityMode == 0){
            return -1;
        }
        $humidityYellowRange = GetValue($this->ReadPropertyFloat('RelativeHumidityYellowRange'));
        $humidityRedRange = GetValue($this->ReadPropertyFloat('RelativeHumidityRedRange'));
